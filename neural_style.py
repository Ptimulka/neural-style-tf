...
def parse_args():
  ...
  parser.add_argument('--device', type=str, 
    default='/gpu:0',
    choices=['/gpu:0', '/cpu:0', 'tpu'],
    help='GPU, TPU or CPU mode.  GPU mode requires NVIDIA CUDA. (default|recommended: %(default)s)')
...
def stylize_with_sess(sess, content_img, style_imgs, init_img, frame):
  # setup network
...
def stylize(content_img, style_imgs, init_img, frame=None):
  if args.device == 'tpu':
    ...
    with tf.Session(tpu_address) as sess:
      stylize_with_sess(sess, content_img, style_imgs, init_img, frame)
  else:
